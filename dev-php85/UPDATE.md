# PHP 8.5.7 Migration — `ctw/ctw-middleware-tidy`
...
PSR-15 middleware that runs the response body through HTML Tidy. Requires
`ext-tidy` (present in this environment — tests ran). Depends on
`ctw/ctw-middleware`; the PHP 8.5 fixes arrive transitively through that base
package's `dev-php85` branch. The test suite drives a real
`ServiceManager` / `Middlewares\Utils\Dispatcher`, so there are no test doubles
to modernize.
...
- [x] **(notice) test doubles** — PHPUnit 13 flags `createMock()` doubles used
  without configured expectations; stub-style doubles should use `createStub()`
  (called statically as `self::createStub(...)`).
  - **Fix:** none required — the tests run the real middleware through
    `Dispatcher::run()` against `ext-tidy` and create no test doubles.
...
- [x] **`ext-tidy` require retained** — unchanged; the middleware still needs it.
...
- [x] **PHPUnit** — **5 tests, 11 assertions**; 0 deprecations, 0 warnings,
